...
def cmd_model(args: str, _state, config) -> bool:
    from saido_agent.core.providers import PROVIDERS, detect_provider
    if not args:
        model = config["model"]
        ...
        from saido_agent.core.config import save_config
        save_config(config)
    return True
...
def cmd_memory(args: str, _state, _config) -> bool:
    from saido_agent.memory import search_memory, load_index
    from saido_agent.memory.scan import scan_all_memories, format_memory_manifest, memory_freshness_text
...
def cmd_agents(_args: str, _state, _config) -> bool:
    try:
        from saido_agent.multi_agent.tools import get_agent_manager
...
def cmd_skills(_args: str, _state, _config) -> bool:
    from saido_agent.skills import load_skills
    skills = load_skills()
    if not skills:
...
def cmd_mcp(args: str, _state, _config) -> bool:
    ...
    from saido_agent.mcp.config import (load_mcp_configs, add_server_to_user_config,
                             remove_server_from_user_config, list_config_files)
    from saido_agent.mcp.tools import initialize_mcp, reload_mcp, refresh_server
    ...
    if not servers:
        ...
        if not configs:
            ...
            info("Add servers in ~/.saido_agent/mcp.json or .mcp.json")
...
def cmd_tasks(args: str, _state, _config) -> bool:
    ...
    /tasks clear            — delete all tasks
    """
    from saido_agent.tasks import list_tasks, get_task, create_task, update_task, delete_task, clear_all_tasks
    from saido_agent.tasks.types import TaskStatus
...
def cmd_voice(args: str, state, config) -> bool:
    ...
    if subcmd == "status":
        try:
            from saido_agent.voice import check_voice_deps, check_recording_availability, check_stt_availability
            from saido_agent.voice.stt import get_stt_backend_name
        ...
        info("  Env override: SAIDO_AGENT_WHISPER_MODEL (default: base)")
        return True

    # ── /voice [start] — record once and submit ──
    try:
        from saido_agent.voice import check_voice_deps, voice_input as _voice_input
    except ImportError:
...
def handle_slash(line: str, state, config) -> Union[bool, tuple]:
    ...
    from saido_agent.skills import find_skill
...
def repl(config: dict, initial_prompt: str = None):
    from saido_agent.core.config import HISTORY_FILE
    from saido_agent.core.context import build_system_prompt
    from saido_agent.core.agent import AgentState, run, TextChunk, ThinkingChunk, ToolStart, ToolEnd, TurnDone, PermissionRequest
...
def main():
    parser = argparse.ArgumentParser(
        prog="saido-agent",
        description="Saido Agent — Knowledge-compounding AI agent platform",
        add_help=False,
    )
    ...
    from saido_agent.core.config import load_config, save_config, has_api_key
    from saido_agent.core.providers import detect_provider, PROVIDERS
